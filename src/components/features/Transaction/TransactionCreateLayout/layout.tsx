...
import { addTransaction } from '@/lib/thunk/transaction/transactionThunk';
import { addTransactionDetailForm, calculateBalance, calculateServicePrice, calculateTotalPrice, removeTransactionDetailForm, setTransactionDetailField, setTransactionOverviewField } from '@/redux/slices/transactionSlice';

//utils
import { formatRupiah } from '@/utils/helpers/formatters/rupiah';
import { CreateTransactionDetail, CreateTransaction } from '@/models/transaction.model';
...
function CreateTransaksiForm() {
  ...
  const {transactionOverview, transactionDetail, status, loading} = useSelector((state: RootState) => state.transaksi)
  ...
  const updateClothingType = (value: string | object, key: keyof CreateTransactionDetail, index: number) => {
    ...
    if(selectedClothing) {
      dispatch(setTransactionDetailField({index, key, value: selectedClothing}))
    }
