...
import { getSingleTransaction, updateTransaction } from '@/lib/thunk/transaction/transactionThunk'
import { RootState } from '@/redux/store'
import { getApparel } from '@/lib/thunk/apparel/apparelThunk'
import { formatRupiah } from '@/utils/helpers/formatters/rupiah'
import { Transaction, TransactionDetail, CreateTransaction, CreateTransactionDetail } from '@/models/transaction.model'
import { setTransactionOverviewField, calculateBalance, calculateServicePrice, calculateTotalPrice, addTransactionDetailForm, removeTransactionDetailForm, setTransactionDetailField, clearStatus } from '@/redux/slices/transactionSlice'
                  ...
                  <Box width={"40%"}>
                    <Table.Root size="1" variant='surface'>
                      ...
                      <Table.Body>
                        {transactionDetail.map((detail, index) => (
                          detail && (
                            <Table.Row key={index}>
                              ...
                              <Table.RowHeaderCell>{detail.jenis_pakaian.jenis_pakaian}</Table.RowHeaderCell>
                        ...
                        <Table.Row>
                          <Table.Cell colSpan={2} align='center' className='font-bold text-yellow-300'>Total</Table.Cell>
