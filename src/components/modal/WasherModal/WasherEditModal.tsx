...
import { updateWasher } from '@/lib/thunk/washer/washerThunk'
import { setWasherForm } from '@/redux/slices/washerSlice';
...
const WasherEditModal = ({data}: {data: Washer}) => {
  const [open, setOpen] = useState(false)
  const [disabled, setDisabled] = useState(false)
  const [showConfirm, setShowConfirm] = useState(false)

  const dispatch = useDispatch<AppDispatch>()
  const {washerForm, status} = useSelector((state:RootState) => state.washer)
