...
import { updateApparel } from '@/lib/thunk/apparel/apparelThunk'
import { setApparelForm } from '@/redux/slices/apparelSlice';
...
const ApparelEditModal = ({data}: {data: Apparel}) => {
  const [open, setOpen] = useState(false)
  const [disabled, setDisabled] = useState(false)
  const [showConfirm, setShowConfirm] = useState(false)

  const dispatch = useDispatch<AppDispatch>()
  const {apparelForm, status} = useSelector((state:RootState) => state.apparel)
...
export default ApparelEditModal